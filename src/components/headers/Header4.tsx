...
const Header4: React.FC<HeaderProps> = ({
  siteName,
  logoUrl,
  buttonText,
  scrollToTop,
  ...
  return (
    ...
    >
      <div className="mx-auto flex flex-wrap items-center justify-between py-4">
        ...
        <div
          onClick={() => scrollToTop()}
          className="order-2 flex cursor-pointer items-center space-x-3 rtl:space-x-reverse"
        >
          {logoUrl ? (
            <img
              src={logoUrl}
        ...
        >
          <ul className="mt-4 flex w-full flex-col rounded-lg border border-gray-100 bg-gray-50 p-4 font-medium max-md:mt-5 md:mt-0 md:flex-row md:items-center md:border-0 md:bg-background md:p-0 lg:space-x-8 rtl:space-x-reverse">
            {navLinks?.map((link, index) => (
              <li key={index}>
                <button
                  onClick={() => linkClicked(link.toLowerCase())}
                  className="lg:text-md block w-full rounded bg-red-500 py-2 capitalize text-gray-900 max-md:text-start max-md:hover:bg-gray-100 sm:text-sm md:bg-transparent md:p-0 md:hover:text-primary-var-500"
                >
                  {link}
                </button>
              </li>
