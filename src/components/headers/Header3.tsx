...
const Header3: React.FC<HeaderProps> = ({
  ...
  return (
    <header
      className={`fixed start-0 top-0 z-[105] w-full border-gray-200 bg-background bg-opacity-80 px-5 md:py-4`}
    >
      <div className="relative mx-auto flex max-w-screen-xl flex-wrap items-center p-4 max-md:justify-between">
        ...
        <button
          type="button"
          className="absolute right-0 hidden space-x-3 rounded-full bg-primary-var-600 px-7 py-2.5 text-center text-sm font-semibold text-white shadow-lg outline-none hover:bg-primary-var-700 md:order-2 md:flex"
          onClick={() => {
            scrollToTop();
            buttonAction();
          }}
        ...
        >
          <ul className="mt-4 flex w-full flex-col rounded-lg border border-gray-100 bg-gray-50 p-4 font-medium max-md:mt-5 md:ml-14 md:mt-0 md:flex-row md:items-center md:space-x-4 md:border-0 md:bg-white md:p-0 lg:space-x-8 rtl:space-x-reverse">
            ...
            {/* Action Button */}
            <li className="mt-4 md:mt-0 md:hidden">
              <button
                type="button"
                className="w-full rounded-full bg-primary-var-600 px-7 py-2 text-center text-sm font-semibold text-white hover:bg-primary-var-700 focus:outline-none focus:ring-4 focus:ring-primary-var-300 md:w-auto"
                onClick={() => {
                  toggleMenu();
                  scrollToTop();
                  buttonAction();
